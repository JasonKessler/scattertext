...
class TestHTMLVisualizationAssembly(TestCase):
    ...
    def test_height_width_nondefault(self):
        ...
        self.assertEqual((HTMLVisualizationAssembly(visualization_data, width_in_pixels=1000, height_in_pixels=60)
                          ._call_build_visualization_in_javascript()),
    ...
    def test_full_data(self):
        ...
        self.assertEqual((HTMLVisualizationAssembly(visualization_data, full_data=full_data)
                          ._call_build_visualization_in_javascript()),
                         self.get_params({17: full_data}))

    def test_show_top_terms(self):
        visualization_data = self.make_adapter()
        self.assertEqual((HTMLVisualizationAssembly(visualization_data, show_top_terms=False)
                          ._call_build_visualization_in_javascript()),
                         self.get_params({18: 'false'}))
        visualization_data = self.make_adapter()
        self.assertEqual((HTMLVisualizationAssembly(visualization_data, show_top_terms=True)
                          ._call_build_visualization_in_javascript()),
    ...
    def test_get_tooltip_content(self):
        ...
        self.assertEqual((HTMLVisualizationAssembly(visualization_data, get_tooltip_content=f)
                          ._call_build_visualization_in_javascript()),
                         self.get_params({20: f}))

    def test_x_axis_labels(self):
        visualization_data = self.make_adapter()
        self.assertEqual((HTMLVisualizationAssembly(visualization_data, x_axis_values=[1, 2, 3])
                          ._call_build_visualization_in_javascript()),
                         self.get_params({21: "[1, 2, 3]"}))

    def test_x_axis_labels(self):
        visualization_data = self.make_adapter()
        self.assertEqual((HTMLVisualizationAssembly(visualization_data, y_axis_values=[4, 5, 6])
                          ._call_build_visualization_in_javascript()),
    ...
    def test_color_func(self):
        ...
        self.assertEqual((HTMLVisualizationAssembly(visualization_data, color_func=color_func)
                          ._call_build_visualization_in_javascript()),
                         self.get_params({23: color_func}))

    def test_show_axes(self):
        visualization_data = self.make_adapter()
        self.assertEqual((HTMLVisualizationAssembly(visualization_data, show_axes=False)
                          ._call_build_visualization_in_javascript()),
                         self.get_params({24: 'false'}))

    def test_show_extra(self):
        visualization_data = self.make_adapter()
        self.assertEqual((HTMLVisualizationAssembly(visualization_data, show_extra=True)
                          ._call_build_visualization_in_javascript()),
                         self.get_params({25: 'true'}))

    def test_do_censor_points(self):
        visualization_data = self.make_adapter()
        self.assertEqual((HTMLVisualizationAssembly(visualization_data, do_censor_points=False)
                          ._call_build_visualization_in_javascript()),
                         self.get_params({26: 'false'}))

    def test_center_label_over_points(self):
        visualization_data = self.make_adapter()
        self.assertEqual((HTMLVisualizationAssembly(visualization_data, center_label_over_points=True)
                          ._call_build_visualization_in_javascript()),
                         self.get_params({27: 'true'}))

    def test_x_axis_labels_over_points(self):
        visualization_data = self.make_adapter()
        self.assertEqual((HTMLVisualizationAssembly(visualization_data, x_axis_labels=['Lo', 'Hi'])
                          ._call_build_visualization_in_javascript()),
                         self.get_params({28: '["Lo", "Hi"]'}))

    def test_y_axis_labels_over_points(self):
        visualization_data = self.make_adapter()
        self.assertEqual((HTMLVisualizationAssembly(visualization_data, y_axis_labels=['Lo', 'Hi'])
                          ._call_build_visualization_in_javascript()),
                         self.get_params({29: '["Lo", "Hi"]'}))

    def test_topic_model_preview_size(self):
        visualization_data = self.make_adapter()
        self.assertEqual((HTMLVisualizationAssembly(visualization_data, topic_model_preview_size=20)
                          ._call_build_visualization_in_javascript()),
                         self.get_params({30: '20'}))

    def test_vertical_lines(self):
        visualization_data = self.make_adapter()
        params = (HTMLVisualizationAssembly(visualization_data, vertical_lines=[20, 31])
                  ._call_build_visualization_in_javascript())
    ...
    def test_horizontal_line_y_position(self):
        visualization_data = self.make_adapter()
        params = (HTMLVisualizationAssembly(visualization_data, horizontal_line_y_position=0)
                  ._call_build_visualization_in_javascript())
        self.assertEqual(params, self.get_params({32: '0'}))

    def test_vertical_line_x_position(self):
        visualization_data = self.make_adapter()
        params = (HTMLVisualizationAssembly(visualization_data, vertical_line_x_position=3)
                  ._call_build_visualization_in_javascript())
