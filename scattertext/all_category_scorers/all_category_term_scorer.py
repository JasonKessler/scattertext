...
class AllCategoryTermScorer(AllCategoryScorer):
    ...
    def get_rank_freq_df(self) -> pd.DataFrame:
        ...
        freq_df = self.term_ranker_.get_ranks(label_append='')
        dfs = []
        for category in self.corpus_.get_categories():
            cat_df = pd.DataFrame({
                'Score': self.term_scorer_.set_categories(category).get_scores().sort_values(ascending=False),
                'Category': category,
            })
            cat_df.index = cat_df.index.rename('Term')
            cat_df = cat_df.reset_index().assign(
                Rank=lambda df: np.arange(len(df), dtype=int),
                Frequency=lambda df: freq_df.loc[df.Term.values, category].values
            )
            dfs.append(cat_df)
